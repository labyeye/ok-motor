...
const ServiceHistory = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const serviceResponse = await axios.get(
          `https://ok-motor-f24l.vercel.app/api/service-bills?page=${currentPage}`,
          {
            headers: {
        ...
        const purchaseResponse = await axios.get(
          `https://ok-motor-f24l.vercel.app/api/buy-letters`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
  ...
  const handleDownload = async (billId) => {
    try {
      const response = await axios.get(
        `https://ok-motor-f24l.vercel.app/api/service-bills/${billId}/download`,
        {
          responseType: "blob",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this service bill?")) {
      try {
        await axios.delete(`https://ok-motor-f24l.vercel.app/api/service-bills/${id}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        });
