...
const createUpdate = asyncHandler(async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: error.message || 'Failed to create update' });
  }
});
...
const getUpdate = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (!user || user.role !== 'admin') return res.status(403).json({ success: false, message: 'Not authorized' });
    return res.json({ success: true, data: update });
  } catch (err) {
